...
/* public api */
int gtls_sslctx_init(SSL_CTX *ctx, const char *crt);
void gtls_sslctx_uninit(SSL_CTX *ctx);
void gtls_ssl_close(SSL *ssl);
int gtls_ssl_accept(SSL **ssl,
                    SSL_CTX *ssl_ctx,
                    int sock,
                    struct mg_context *phys_ctx);
int gtls_ssl_read(SSL *ssl, unsigned char *buf, size_t len);
int gtls_ssl_write(SSL *ssl, const unsigned char *buf, size_t len);
...
gtls_sslctx_init(SSL_CTX *ctx, const char *crt)
{
	...
	if (rc != GNUTLS_E_SUCCESS) {
		DEBUG_TRACE("Failed to allocate credentials (%d): %s", rc, gnutls_strerror(rc));
		goto failed;
	}

	rc = gnutls_priority_init(&ctx->prio, NULL, NULL);
	if (rc != GNUTLS_E_SUCCESS) {
		DEBUG_TRACE("Failed to allocate priority cache (%d): %s", rc, gnutls_strerror(rc));
		goto failed;
	}
	...
	if (rc != GNUTLS_E_SUCCESS) {
		DEBUG_TRACE("TLS parse crt/key file failed (%d): %s", rc, gnutls_strerror(rc));
		goto failed;
	}
                ...
                struct mg_context *phys_ctx)
{
	...
	*ssl = (SSL*)mg_calloc_ctx(1, sizeof(SSL), phys_ctx);
	...
	if (rc != GNUTLS_E_SUCCESS) {
		DEBUG_TRACE("Failed to initialize session (%d): %s", rc, gnutls_strerror(rc));
		goto failed;
	}

	rc = gnutls_priority_set((*ssl)->sess, ssl_ctx->prio);
	if (rc != GNUTLS_E_SUCCESS) {
		DEBUG_TRACE("TLS set priortities failed (%d): %s", rc, gnutls_strerror(rc));
		goto failed;
	}

	rc = gnutls_credentials_set((*ssl)->sess, GNUTLS_CRD_CERTIFICATE, ssl_ctx->cred);
	if (rc != GNUTLS_E_SUCCESS) {
		DEBUG_TRACE("TLS set credentials failed (%d): %s", rc, gnutls_strerror(rc));
		goto failed;
	}

	gnutls_certificate_send_x509_rdn_sequence((*ssl)->sess, 1);
	gnutls_certificate_server_set_request((*ssl)->sess, GNUTLS_CERT_IGNORE);
	gnutls_handshake_set_timeout((*ssl)->sess, GNUTLS_DEFAULT_HANDSHAKE_TIMEOUT);
		...
		if (gnutls_error_is_fatal(rc)) {
			if (rc == GNUTLS_E_FATAL_ALERT_RECEIVED) {
				DEBUG_TRACE("TLS fatal alert received: %s",
				            gnutls_alert_get_name(gnutls_alert_get((*ssl)->sess)));
			} else {
				DEBUG_TRACE("TLS handshake failed (%d): %s", rc, gnutls_strerror(rc));
			}
